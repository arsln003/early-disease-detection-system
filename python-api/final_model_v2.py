...
def preprocess(img_path, img_size=256):
    img_np = np.array(
        Image.open(img_path).convert('L').resize((img_size, img_size)),
        dtype=np.float32
    ) / 255.0

    img_t = torch.from_numpy(
        np.stack([img_np] * 3, axis=0)
    ).unsqueeze(0)  # (1, 3, H, W)
...
def predict(
    ...
    # ── 1. CLASSIFICATION ─────────────────────────────────────────────────────
    with torch.no_grad():
        probs = torch.softmax(classifier_model(img_t), dim=1)[0].cpu().numpy()
    ...
    if pred_class in (1, 2):
        with torch.no_grad():
            logits   = segmenter_model(img_t)
            seg_mask = logits.argmax(1)[0].cpu().numpy()

        lesion_px  = int((seg_mask > 0).sum())
        lesion_pct = 100 * lesion_px / (img_size * img_size)
        print(f"\n── SEGMENTATION ────────────────────────────────────")
        print(f"   Lesion pixels : {lesion_px}  ({lesion_pct:.2f}%)")
        print(f"   Classes found : {set(int(c) for c in np.unique(seg_mask))}")
    else:
        print("\n   No stroke detected — skipping segmentation.")

    # ── 3. VISUALISE & SAVE ───────────────────────────────────────────────────
    visualise_and_save(img_np, seg_mask, pred_class, confidence, img_path, out_dir)
